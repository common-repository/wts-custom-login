...
if ( !class_exists( 'WTS_Custom_Login' ) ) {
	class WTS_Custom_Login {    
		...
		*/
		public function __construct() {
			...
			add_action( 'admin_enqueue_scripts', array( $this, 'wts_clp_footer_script' ) );
		...
		*/
		public function wts_clp_load_scripts( $hook ) {
			...
			$custoLoginPageSlug	= get_option('wts_clp_login_page_name');
		...
		*/
		public function require_global() {
			...
			//load page on the fly or to create virtual page/post
			require plugin_dir_path( __FILE__ ) . 'includes/global/wcl_onthefly_login_page.php';
		}
		...
		*/
		...
		function wts_clp_footer_script( $hook ) {
			//this will add this js file plugin's settings page 'wts-custom-login-options'
			if( $hook == 'toplevel_page_wts-custom-login-options') {			
				//Core media script
				wp_enqueue_media();
				...
				wp_enqueue_script( 'wp-media-chooser-jquery', plugins_url( 'assets/js/wts-clp-choose-media.js', $this->file ), array( 'jquery' ), $this->version,true );		
			}	
		}
	...
	*/
	...
	function wts_custom_login_plugin_activation_hook() {
		...
		$loginPageId	= -42;
		$loginPageSlug	= 'user-login';
		
		update_option( 'wts_clp_login_page_name', $loginPageSlug, 'yes' );
		update_option( 'wts_clp_login_page_id', $loginPageId, 'yes' );		
	}
	...
	*/
	...
	function wts_custom_login_plugin_deactivation_hook() {
		...
		//delete on the fly page 
		delete_option( 'wts_clp_login_page_name' );
		delete_option( 'wts_clp_login_page_id' );			
	}
