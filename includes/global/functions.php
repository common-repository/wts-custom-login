...
function wts_clp_loginarea_inline_style_load( $hook ) {	
	...
	*/
	...
	//Add css only for this plugin options page.
	...
	if( is_page( $custoLoginPageSlug ) ){ 
		...
		//options page styles 
		wp_enqueue_style(
			'wts-clp-plugin-login-pageinline-styles',
			plugin_dir_url( wts_clp()->file ) . 'wcl-options/assets/css/wts-options.css'
		);	
		...
		//add this custom css as inline css
		wp_add_inline_style( 'wts-clp-plugin-login-pageinline-styles', $customCssFrontPage );	
	}
}
